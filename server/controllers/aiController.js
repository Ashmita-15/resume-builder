...
export const generateCoverLetter = async (req, res) => {
    try {
        const { jobDescription, resumeData, resumeText } = req.body;

        if(!jobDescription || (!resumeData && !resumeText)){
            return res.status(400).json({message: 'Missing required fields'})
        }

        const systemPrompt = "You are an expert career assistant. Write a tailored, concise, ATS-friendly cover letter (180-250 words). Use first person and a professional, warm tone. Structure: greeting, alignment with role/company, 1-2 short paragraphs tying specific resume achievements to JD needs, and a closing with a call to action. Return plain text only.";

        const userPrompt = resumeData
